...
class MCB2P_NEW_STOCK_TABLE (Base):
	...
	def __str__():
		return "<MCB2P_NEW_STOCK_TABLE(UNAME= '%s', ITEM_NAME= '%s',QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE,self.QUANTITY, self.DATE,self.MONTH)

class MCB2P_STOCK_FLOW_TABLE (Base):
	...
	def __str__():
		return "<MCB2P_STOCK_FLOW_TABLE(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s', QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE,self.QUANTITY, self.DATE,self.MONTH)

class MCB2P_STATUS_TABLE (Base):
	...
	def __str__():
		return "<MCB2P_STOCK_FLOW_TABLE(ITEM_NAME= '%s',QUANTITY='%s')>"%(self.ITEM_NAME, self.QUANTITY)


#  connect dispesary.db
db_connection = create_engine('sqlite:///data_bases/chint/mcb_2p.db')
if not db_connection:
	print("\t \t ERROR :: No connection to MCB2P database")
else:
	print("\t \t SUCCESS :: MCB2P database connected well ")
