...
class CABLES_NEW_STOCK_TABLE (Base):
	...
	def __str__():
		return "<CABLES_NEW_STOCK_TABLE(UNAME= '%s', ITEM_NAME= '%s',QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE,self.QUANTITY, self.DATE,self.MONTH)

class CABLES_STOCK_FLOW_TABLE (Base):
...
class CABLES_STATUS_TABLE (Base):
	...
	def __str__():
		return "<CABLES_STOCK_FLOW_TABLE(ITEM_NAME= '%s',QUANTITY='%s')>"%(self.ITEM_NAME, self.QUANTITY)


#  connect dispesary.db
db_connection = create_engine('sqlite:///data_bases/chint/cables.db')
if not db_connection:
	print("\t \t ERROR :: No connection to CABLES database")
else:
	print("\t \t SUCCESS :: CABLES database connected well ")
