...
class Dampshild_New_stock_table (Base):
	...
	def __str__():
		return "<Dampshild_New_stock_table(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s', INVOICE= '%s', QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE, self.INVOICE, self.QUANTITY, self.DATE,self.MONTH)

class Dampshild_stock_flow_table (Base):
	...
	def __str__():
		return "<Dampshild_stock_flow_table(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s', QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE, self.INVOICE, self.QUANTITY, self.DATE,self.MONTH)

class Dampshild_status_table (Base):
    ...
    def __str__():
        return "<Dampshild_status_table(ITEM_NAME= '%s', FOUR_LTR = '%s', ONE_LTR= '%s' )>"%(self.ITEM_NAME,  self.FOUR_LTR, self.ONE_LTR)


#  connect dispesary.db
db_connection = create_engine('sqlite:///data_bases/sadolin/dampshild.db')
if not db_connection:
	print("\t \t ERROR :: No connection to Dampshild database")
else:
	print("\t \t SUCCESS :: Dampshild database connected well ")
