...
class Wall_guard_New_stock_table (Base):
	...
	def __str__():
		return "<Wall_guard_New_stock_table(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s', INVOICE= '%s', QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE, self.INVOICE, self.QUANTITY, self.DATE,self.MONTH)

class Wall_guard_stock_flow_table (Base):
	...
	def __str__():
		return "<Wall_guard_stock_flow_table(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s', QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE, self.INVOICE, self.QUANTITY, self.DATE,self.MONTH)

class Wall_guard_status_table (Base):
    ...
    def __init__(self, ITEM_NAME, TWENTY_LTR, FOUR_LTR):
        self.ITEM_NAME = ITEM_NAME
        self.TWENTY_LTR = TWENTY_LTR
        self.FOUR_LTR = FOUR_LTR 

    def __str__():
        return "<Wall_guard_status_table(ITEM_NAME= '%s', TWENTY_LTR = '%s', FOUR_LTR= '%s' )>"%(self.ITEM_NAME,  self.TWENTY_LTR, self.FOUR_LTR)


#  connect dispesary.db
db_connection = create_engine('sqlite:///data_bases/sadolin/wall_guard.db')
if not db_connection:
	print("\t \t ERROR :: No connection to Wall_guard database")
else:
	print("\t \t SUCCESS :: Wall_guard database connected well ")
