...
Tables
		++++++
			...
			|
			|---> Weather_guard_stock_flow_table
...
class Weather_guard_New_stock_table (Base):
	...
	def __str__():
		return "<Weather_guard_New_stock_table(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s', INVOICE= '%s', QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE, self.INVOICE, self.QUANTITY, self.DATE,self.MONTH)

class Weather_guard_stock_flow_table (Base):
	...
	def __str__():
		return "<Weather_guard_stock_flow_table(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s', QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE, self.INVOICE, self.QUANTITY, self.DATE,self.MONTH)

class Weather_guard_status_table (Base):
	...
	def __str__():
		return "<Weather_guard_status_table(ITEM_NAME= '%s',TWENTY_LTR='%s' FOUR_LTR= '%s', ONE_LTR = '%s')>"%(self.ITEM_NAME, self.TWENTY_LTR,self.FOUR_LTR, self.ONE_LTR)


#  connect dispesary.db
db_connection = create_engine('sqlite:///data_bases/sadolin/weather.db')
if not db_connection:
	print("\t \t ERROR :: No connection to Weather_guard database")
else:
	print("\t \t SUCCESS :: Weather_guard database connected well ")
