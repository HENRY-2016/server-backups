...
class Road_marking_New_stock_table (Base):
	...
	def __str__():
		return "<Road_marking_New_stock_table(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s', INVOICE= '%s', QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE, self.INVOICE, self.QUANTITY, self.DATE,self.MONTH)

class Road_marking_stock_flow_table (Base):
	...
	def __str__():
		return "<Road_marking_stock_flow_table(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s', QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE, self.INVOICE, self.QUANTITY, self.DATE,self.MONTH)

class Road_marking_status_table (Base):
	...
	def __init__(self, ITEM_NAME, ONE_LTR, FOUR_LTR):
		self.ITEM_NAME = ITEM_NAME
		self.ONE_LTR = ONE_LTR 
		self.FOUR_LTR = FOUR_LTR
		
	def __str__():
		return "<Road_marking_status_table(ITEM_NAME= '%s', ONE_LTR= '%s', FOUR_LTR = '%s')>"%(self.ITEM_NAME, self.ONE_LTR, self.FOUR_LTR)


#  connect dispesary.db
db_connection = create_engine('sqlite:///data_bases/sadolin/road_marking.db')
if not db_connection:
	print("\t \t ERROR :: No connection to Road_marking database")
else:
	print("\t \t SUCCESS :: Road_marking database connected well ")
