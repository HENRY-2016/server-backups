...
class Emulsion_New_stock_table (Base):
	...
	def __str__():
		return "<Emulsion_New_stock_table(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s', INVOICE= '%s', QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE, self.INVOICE, self.QUANTITY, self.DATE,self.MONTH)

class Emulsion_stock_flow_table (Base):
	...
	def __str__():
		return "<Emulsion_stock_flow_table(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s', QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE, self.INVOICE, self.QUANTITY, self.DATE,self.MONTH)

class Emulsion_status_table (Base):
	...
	def __str__():
		return "<Emulsion_status_table(ITEM_NAME= '%s', TWENTY_LTR= '%s', FOUR_LTR = '%s')>"%(self.ITEM_NAME, self.TWENTY_LTR, self.FOUR_LTR)


#  connect dispesary.db
db_connection = create_engine('sqlite:///data_bases/sadolin/emulsion.db')
if not db_connection:
	print("\t \t ERROR :: No connection to Emulsion database")
else:
	print("\t \t SUCCESS :: Emulsion database connected well ")
