...
Tables
		++++++
			...
			|
			|---> Clear_varnish_stock_flow_table
...
class Clear_varnish_New_stock_table (Base):
	...
	def __str__():
		return "<Clear_varnish_New_stock_table(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s', INVOICE= '%s', QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE, self.INVOICE, self.QUANTITY, self.DATE,self.MONTH)

class Clear_varnish_stock_flow_table (Base):
	...
	def __str__():
		return "<Clear_varnish_stock_flow_table(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s', QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE, self.INVOICE, self.QUANTITY, self.DATE,self.MONTH)

class Clear_varnish_status_table (Base):
    ...
    def __str__():
        return "<Clear_varnish_status_table(ITEM_NAME= '%s', FOUR_LTR = '%s', ONE_LTR= '%s' )>"%(self.ITEM_NAME,  self.FOUR_LTR, self.ONE_LTR)


#  connect dispesary.db
db_connection = create_engine('sqlite:///data_bases/sadolin/clear_varnish.db')
if not db_connection:
	print("\t \t ERROR :: No connection to Clear_varnish database")
else:
	print("\t \t SUCCESS :: Clear_varnish database connected well ")
