...
class AdesilexP7_New_stock_table (Base):
	...
	def __str__():
		return "<AdesilexP7_New_stock_table(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s',QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE,self.QUANTITY, self.DATE,self.MONTH)

class AdesilexP7_stock_flow_table (Base):
	...
	def __str__():
		return "<AdesilexP7_stock_flow_table(UNAME= '%s', ITEM_NAME= '%s', SIZE= '%s', QUANTITY = '%s',DATE='%s',MONTH='%s')>"%(self.UNAME, self.ITEM_NAME, self.SIZE,self.QUANTITY, self.DATE,self.MONTH)

class AdesilexP7_status_table (Base):
	...
	def __str__():
		return "<AdesilexP7_status_table(ITEM_NAME= '%s',QUANTITY='%s')>"%(self.ITEM_NAME, self.QUANTITY)


#  connect dispesary.db
db_connection = create_engine('sqlite:///data_bases/mapei/adesilexP7.db')
if not db_connection:
	print("\t \t ERROR :: No connection to AdesilexP7 database")
else:
	print("\t \t SUCCESS :: AdesilexP7 database connected well ")
