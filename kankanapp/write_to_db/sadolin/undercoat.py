...
@app.route('/undercoat_record_new_stock',methods=['POST'])
def undercoat_record_new_stock ():
    ...
    if  request.method == 'POST':
        ...
        new_stock = Undercoat_New_stock_table (uname,paint_name,size,invoce_no,qty,Date,current_month)
        session_query.add(new_stock)
        session_query.commit() 
        #==========================
        paint_names_in_db = session_query.query(Undercoat_status_table ).order_by(Undercoat_status_table.ITEM_NAME)
        paint_names_list  = []
        ...
        if (paint_name in paint_names_list): # do updating....
            row_2_update = session_query.query(Undercoat_status_table ).filter_by(ITEM_NAME = paint_name)
            ...
            if size == four_ltr:
                ...
                session_query.query(Undercoat_status_table).filter_by(ITEM_NAME = paint_name).update({"FOUR_LTR":new_quantity_value})			
                session_query.commit()

            elif size == twenty_ltr:
                for quantity in row_2_update: quantity_in_db = quantity.TWENTY_LTR # get quantity value
                new_quantity_value = int(quantity_in_db) + int(qty)
                session_query.query(Undercoat_status_table).filter_by(ITEM_NAME = paint_name).update({"TWENTY_LTR":new_quantity_value})			
                session_query.commit()

        elif (paint_name not in paint_names_list):
            if (paint_name not in paint_names_list) and (size == four_ltr):
                sql = Undercoat_status_table(paint_name,0,qty)
                session_query.add(sql)
                session_query.commit()
            elif (paint_name not in paint_names_list) and (size == twenty_ltr):
                sql = Undercoat_status_table(paint_name,qty,0)
                session_query.add(sql)
                session_query.commit()
...
@app.route ('/undercoat_record_used_paint',methods=['POST'])
def undercoat_record_used_paint ():
    DBsession = sessionmaker(bind=undercoat_db_connection)
    session_query = DBsession()
    mth = datetime.datetime.today()
    mth2 = str(mth.month) 
    ...
    if request.method == "POST":
        ...
        recorded_paint = Undercoat_stock_flow_table (uname,paint_name,size,qty,Date,current_month)
